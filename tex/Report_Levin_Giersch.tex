...
\usepackage{subcaption}                % multi-panel figures (supersedes the deprecated subfigure package)
...
\author{Levin Damian Antonin Giersch}
...
\abstract{
Transient paleoclimate simulations and reconstructions are indispensable for understanding how the Earth system responds to forcing, yet they are distributed in mutually incompatible formats. Each archive uses a different time origin and unit, a different spatial grid and longitude convention, a different temperature variable, and a different temporal and spatial resolution. This project develops a small, reproducible Python toolchain that downloads four widely used temperature archives covering the last glacial cycle, namely TraCE-21k, CHELSA-TraCE21k-centennial, Beyer2020/2021 and PalMod2. It harmonises their time axes onto a common kiloyears-before-present (\kaBP) scale, reconciles their spatial grids, converts all temperatures to degrees Celsius, and then extracts and compares temperature time series at an arbitrary point as well as globally through pairwise mean-difference heatmaps. Applied to an example location in northern Germany ($\approx 53.26\dg$\,N, $11.67\dg$\,E), the four archives agree on the first-order signal, a cold Last Glacial Maximum (LGM) followed by deglacial warming into the Holocene, but disagree on absolute temperature by several degrees, with the largest systematic divergences at high northern latitudes. The report documents the datasets, the acquisition and pre-processing pipeline, the harmonisation methods and the results, and discusses why these archives are difficult to compare and what the comparison reveals.
}

\keywords{Paleoclimate, Data harmonisation, Paleoclimate datasets, Temperature comparison, TraCE-21k, CHELSA-TraCE21k, PalMod2, Beyer2020}
...
All point-based results below are shown for a single terrestrial location in northern Germany ($\approx 53.26\dg$\,N, $11.67\dg$\,E, south-western Mecklenburg, near the lower Elbe), internally labelled \tabbr{022\_00TG}. The location is used purely as a worked example; the tool accepts any coordinate. It is mid-latitude, continental and ice-free throughout the studied interval, which makes it a convenient case for examining how the four archives represent the deglacial warming at one place.

\section{The data}
Four archives were selected to span the trade-off between temporal resolution, spatial resolution and time coverage. Two are raw transient GCM simulations (TraCE-21k, PalMod2); two are high-resolution, observation-anchored products (Beyer2020/2021, CHELSA-TraCE21k). Table~\ref{tab:overview} summarises them. All are downloaded via the \tabbr{data\_downloader} notebook; the total download is on the order of 1\,TB (about 800\,GB of free disk is recommended), dominated by the $\sim$1\,km CHELSA GeoTIFFs.
...
\midrule
TraCE-21k         & Coupled GCM (CCSM3)         & TSA, TSOI   & 22--0 \kaBP  & decadal    & T31 ($\sim$3.75\dg)   & K \\
PalMod2           & Coupled GCM (MPI-ESM1.2-CR) & tas, tsl    & 25--0 \kaBP  & annual     & T31 ($\sim$3.71\dg)   & K \\
Beyer2020/21 (v4) & Downscaled reconstruction   & temperature & 120--0 \kaBP & 2/1 kyr    & 0.5\dg{}              & \degC \\
CHELSA-TraCE21k   & Downscaled product          & tasmean     & 22--0 \kaBP  & centennial & $\sim$1\,km (30\,arcsec) & K \\
\bottomrule
\end{tabular}
...
CHELSA-TraCE21k  & EnviDat / SwitchCH S3 & Parallel download (32 threads), atomic \tabbr{.part} rename. \\
Beyer2020/2021   & figshare API          & Query API for version 4 of article 12293345. \\
...
Two of the four archives cannot be used as delivered. The \tabbr{data\_processer} notebook performs the necessary conversions; TraCE-21k and Beyer2020 are already analysis-ready NetCDF and are passed through unchanged.

\subsection{CHELSA: GeoTIFF to centennial-mean NetCDF}
The CHELSA centennial product arrives as one GeoTIFF per month, per time index, per variable, named \tabbr{CHELSA\_TraCE21k\_<var>\_<MM>\_<TTT>\_V.1.0.tif}, where \tabbr{MM} is the month and \tabbr{TTT} a centennial time index. The processor first groups files by year: a regex extracts month and index, and the index is mapped to a calendar year by $\mathrm{year} = 1990 - (20 - \mathrm{TTT})\times100$, anchored at $\mathrm{TTT}=20 \rightarrow 1990$\,CE with 100-year steps. Thus index 0 maps to about year $-10$\,CE and index $-200$ to about $-20010$\,CE ($\approx 22$ \kaBP, near the LGM). This mapping is consistent with the file's \tabbr{frequency=centennial} metadata, the $\sim$21,000-year nominal coverage and the file count (221 steps $\times$ 100 years $\approx 22$\,kyr); the exact index-to-year table is not published by CHELSA, so the mapping is a well-constrained reconstruction, flagged as such in the notebook. For each year the 12 monthly rasters are stacked and averaged to one (lat, lon) field; CHELSA stores temperature as Kelvin $\times$ 10, so a factor of 0.1 is applied and the result cast back to \tabbr{float32}. All years are concatenated along a new time axis (one timestamp at 1~July of each year), tagged with CF-style metadata, and written as one gzip-compressed NetCDF per variable at the full native $\sim$1\,km resolution. Finally $\mathrm{tasmean} = (\mathrm{tasmin}+\mathrm{tasmax})/2$ is derived.
...
The four archives cannot be compared until they share a common footing in variable, time, space and unit. This is the analytical core of the project, performed in the comparison notebook.

\subsection{Variables}
The comparison restricts itself to near-surface air temperature (and, separately, soil temperature), the only quantities common to enough archives to be comparable. Even so the variables are not strictly identical (Table~\ref{tab:vars}): TraCE \tabbr{TSA} and PalMod2 \tabbr{tas} are model 2\,m air temperatures; Beyer \tabbr{temperature} is a downscaled, bias-corrected monthly mean; CHELSA \tabbr{tasmean} is the mean of downscaled daily extremes. These are physically close but not interchangeable, a caveat carried into the results. For soil temperature, only PalMod2 and TraCE provide a layered variable; the comparison keeps the topmost soil layer (for TraCE, \tabbr{levsoi}[0] $\approx 0.007$\,m).
...
CHELSA    & tasmean     & --              & K     & centennial clim. \\
\bottomrule
\end{tabular}
...
CHELSA    & \tabbr{days since -20010-07-01} (forward)     & $[0,\,8\,035\,335]$   & $(\max-\mathrm{vals})/365250$ \\
\bottomrule
\end{tabular}
...
\subsection{Space}
The archives sit on four grids (Table~\ref{tab:space}) and use two longitude conventions (0--360\dg{} and $-180$--180\dg). The tool detects latitude and longitude coordinates by name and, before any query, converts a requested negative longitude to 0--360\dg{} when the file demands it. The spatial-resolution mismatch is severe: a single PalMod2 or TraCE cell ($\sim$3.7\dg) is roughly 250\,km $\times$ 410\,km at 53\dg\,N, whereas a CHELSA pixel (0.0083\dg) is about 1\,km and a Beyer cell (0.5\dg) about 55\,km. PalMod2 and TraCE share the same T31 grid, so their cells coincide exactly.
...
CHELSA    & global $\sim$1\,km     & global & global & $-180$--180 & 0.0083\dg \\
\bottomrule
\end{tabular}
\caption{Spatial grids (authoritative native resolutions).}
\label{tab:space}
\end{table*}

\subsection{Units}
PalMod2, TraCE-21k and CHELSA store temperature in kelvin; Beyer2020 in degrees Celsius. All extracted series are converted to \degC{} (subtracting 273.15\,K where required) before plotting or differencing, so every figure is in a single, intuitive unit.
...
Beyond the single-point series, the tool produces global pairwise mean-difference heatmaps. For a pair of archives $A$ and $B$: (1) a \emph{shared time window} is computed from the two time axes (their overlap), read cheaply from the time coordinate alone, so both means cover the same interval; (2) a \emph{common sampling grid} is used, namely every cell centre of the TraCE-21k grid ($96\times48 = 4608$ points), and for each archive the temperature at each point is read with a vectorised nearest-neighbour index, averaged over the shared window and converted to \degC; (3) the difference $A-B$ is drawn on a PlateCarree world map with the diverging PRGn colormap and a TwoSlopeNorm centred at zero (green $=A$ warmer, purple $=A$ colder), and cells where either archive has no data (for example ocean cells for the terrestrial Beyer and CHELSA archives) remain blank.

Because the shared sampling grid is the \emph{coarse} TraCE grid, the high-resolution archives (Beyer at 0.5\dg, CHELSA at $\sim$1\,km) are effectively sub-sampled, one representative pixel per $\sim$3.7\dg{} cell, rather than area-averaged into the coarse cell. This keeps the heatmaps fast and memory-bounded but means they compare \emph{point samples}, not area means (see Section~\ref{sec:caveats}). The point-extraction time-series plots, by contrast, use each archive at its native resolution; in particular CHELSA is read at its full $\sim$1\,km grid, so the returned CHELSA cell matches the requested point to within one $\sim$1\,km pixel ($\approx 0.0083\dg$).
...
Figure~\ref{fig:ts_air} shows that Beyer2020 (blue), the only archive spanning the full 120\,kyr, reproduces the expected glacial-interglacial structure: a warm last interglacial ($\sim$9\,\degC{} around 120\,ka), a long oscillating cooling, a cold interval around the LGM (dropping to $\approx -4$\,\degC{} near 20--25\,ka), and deglacial warming to $\sim$8--9\,\degC{} in the Holocene; its 1--2\,kyr step makes the curve smooth. PalMod2 (orange) and TraCE-21k (green) begin near the LGM and, at their native annual and decadal resolution, expose large high-frequency variability. CHELSA (red) tracks the deglacial warming at centennial resolution and reaches the warmest late-Holocene values here. In the overlapping window the four agree on the shape of the signal but disagree on absolute temperature by several degrees; most conspicuously, TraCE-21k settles to a noticeably cooler Holocene plateau ($\sim$3\,\degC) than the other three ($\sim$7--9\,\degC), an offset of order 4--5\,\degC{} that recurs in the heatmaps below. Part of the offset is a scale artefact: the coarse GCMs return a $\sim$3.7\dg{} cell centred at $\sim$53.8\dg\,N, 11.25\dg\,E, which averages a large, partly maritime region, whereas Beyer and CHELSA resolve the land point itself.
...
\paragraph{Air temperature, all six pairs.} The four air archives yield six pairwise comparisons (Fig.~\ref{fig:diff_air}), each averaged over the shared window noted in the panel. The maps show that Beyer is generally warmer than PalMod2 across the tropics and mid-latitudes but markedly colder over the high-northern continents (Fig.~\ref{fig:diff_air}a); Beyer is warmer than TraCE almost everywhere, most strongly over northern Eurasia (Fig.~\ref{fig:diff_air}b); Beyer and CHELSA, the two observation-anchored products, are the most similar pair, with only faint differences (Fig.~\ref{fig:diff_air}c); the two GCMs differ strongly and coherently, PalMod2 much warmer than TraCE at high northern latitudes and cooler over the Southern Ocean margin (Fig.~\ref{fig:diff_air}d); PalMod2 $-$ CHELSA is moderate and patchy (Fig.~\ref{fig:diff_air}e); and TraCE is colder than CHELSA across most land, again strongest in the high north (Fig.~\ref{fig:diff_air}f).

\begin{figure*}[t]
\centering
\begin{subfigure}{0.49\textwidth}\centering\includegraphics[width=\linewidth]{diff_air_beyer2020_palmod2.png}\caption{Beyer2020 $-$ PalMod2 (0--25\,ka).}\end{subfigure}\hfill
\begin{subfigure}{0.49\textwidth}\centering\includegraphics[width=\linewidth]{diff_air_beyer2020_trace21k.png}\caption{Beyer2020 $-$ TraCE-21k (0--22\,ka).}\end{subfigure}

\vspace{3pt}
\begin{subfigure}{0.49\textwidth}\centering\includegraphics[width=\linewidth]{diff_air_beyer2020_chelsatrace21k.png}\caption{Beyer2020 $-$ CHELSA (0--22\,ka).}\end{subfigure}\hfill
\begin{subfigure}{0.49\textwidth}\centering\includegraphics[width=\linewidth]{diff_air_palmod2_trace21k.png}\caption{PalMod2 $-$ TraCE-21k (0--22\,ka).}\end{subfigure}

\vspace{3pt}
\begin{subfigure}{0.49\textwidth}\centering\includegraphics[width=\linewidth]{diff_air_palmod2_chelsatrace21k.png}\caption{PalMod2 $-$ CHELSA (0--22\,ka).}\end{subfigure}\hfill
\begin{subfigure}{0.49\textwidth}\centering\includegraphics[width=\linewidth]{diff_air_trace21k_chelsatrace21k.png}\caption{TraCE-21k $-$ CHELSA (0--22\,ka).}\end{subfigure}
\caption{Mean air-temperature differences for all six pairs of the four archives (green $=$ first archive warmer, purple $=$ colder), sampled at the TraCE grid and averaged over each pair's shared time window.}
\label{fig:diff_air}
\end{figure*}
...
\section{Future work}
Several extensions would sharpen the analysis. \emph{Automated multi-point sampling} would iterate the extraction over many locations (or whole grids) and aggregate, rather than inspecting one point at a time. \emph{Stratified analysis} would correlate the differences with latitude band, climate zone, land versus ocean, distance to former ice margins and elevation, moving from where the archives differ to why. \emph{Area-weighted regridding} would replace nearest-neighbour sampling in the heatmaps with conservative regridding, so the high-resolution archives are compared as true area means. \emph{Uncertainty} could be carried and displayed as the within-window temporal variability alongside the means, so an apparent offset can be judged against each archive's own variability. Finally, the comparison could be extended beyond temperature to precipitation and to the bioclimatic and vegetation fields available in Beyer2020.
